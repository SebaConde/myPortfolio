...
export default function FeaturedCertificationsCards() {
              ...
              <div>
                <div className="flex justify-between items-start p-4">
                  <div className="flex-1">
                    <motion.h3 layoutId={`title-${active.title}-${id}`} className="font-medium text-neutral-700 dark:text-neutral-200 text-base text-left">
                      {active.title}
                    </motion.h3>
                    <motion.p layoutId={`description-${active.description}-${id}`} className="text-neutral-600 dark:text-neutral-400 text-base text-left">
                      {active.description}
                    </motion.p>
                    <motion.p layoutId={`fecha-${active.fecha}-${id}`} className="text-neutral-500 dark:text-neutral-500 text-xs text-left mt-1 flex items-center gap-1">
                  ...
                  <div className="flex flex-col items-end gap-2 ml-4">
                    ...
                    <button className="relative inline-flex h-6 overflow-hidden rounded-full p-px focus:outline-none "
                      onClick={() => (window.open(active.credencial, "_blank", "noopener,noreferrer"))}>
...
const cards = [
  {
    ...
    content: () => {
      return (
        <div>
            <p>
            Es un programa completo que me dio una base sólida en el desarrollo de aplicaciones web con React JS, una de las bibliotecas más populares y demandadas del ecosistema JavaScript. A lo largo del curso me fueron guiando paso a paso por todos los conceptos fundamentales y pude aprender desde lo básico hasta lo necesario para construir aplicaciones modernas, interactivas y eficientes. 
          </p>
        </div>
      );
    },
  },
  {
    ...
    content: () => {
      return (
        <div>
            <p>
            Desarrollé habilidades en uso práctico de Gemini, creación de prompts efectivos e integración con Google Workspace para automatizar procesos y crear asistentes inteligentes. 
          </p>
        </div>
      );
    },
